...
/*2012 06 26 */
+ (NSMutableArray *)newQuickConvert:(NSString *)hzString {
  ...
  if (cache == NULL) {
    ...
    NSUInteger cacheLength = cacheCeil - cacheFloor + 1;
    //NSLog(@"sizeof(pinyinTableCell *)  %lu" ,sizeof(pinyinTableCell *));
    cache = malloc(cacheLength * sizeof(pinyinTableCell *));
  ...
  NSMutableArray *string_array = [[[NSMutableArray alloc] init] autorelease];
  for (int i = 0; i < hzLength; i++) {
    unichar c = [hzString characterAtIndex:i];
    if (c < 0xff) {
      [string appendFormat:@"%c", c];
    ...
    pinyinTableCell pycell = pinyinTable[pos];
    // [string appendFormat:@"%@", pycell.pinyin];
    [string_array addObject:pycell.pinyin];
  }
...
+ (NSMutableArray *) fetchPinyinTableArray
{
  ...
  for (int pos = 0; pos < 475; pos++) {
    pinyinTableCell pycell = pinyinTable[pos];
    [pinyin_table_array addObject:pycell.pinyin];
  }
  // NSLog(@"%@", pinyin_table_array);
  return pinyin_table_array;
}
